...
const DEMO_EMPLOYEES: EmployeeKpiRow[] = [
  {
    id: '1',
    name: 'James Okello',
    station: 'Nansana Station',
    role: 'Station Attendant',
    department: 'Retail — Stations',
    throughput: 92,
    attendancePct: 98,
    customerScore: 4.8,
    overall: 91,
...
const STATION_LEADERBOARD = [
  { name: 'Nansana Station', score: 94 },
  { name: 'Nairobi West', score: 89 },
  { name: 'Kampala Central', score: 84 },
  { name: 'Mombasa Road', score: 71 },
  { name: 'Jinja Highway', score: 68 },
];

const UPCOMING_REVIEWS = [
  { name: 'Robert Kato', station: 'Kampala Central', due: '2026-05-12' },
  { name: 'Esther Mwikali', station: 'Mombasa Road', due: '2026-05-14' },
  { name: 'Grace Akinyi', station: 'Mombasa Road', due: '2026-05-18' },
];
    ...
    <div className="page-shell space-y-8">
      <header className="page-header !mb-0">
        <div className="min-w-0">
          ...
          <p className="page-description mt-1 max-w-2xl">Track KPIs, appraisals and team scores by station</p>
          <EntityContextBanner />
        </div>
      </header>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <article className="flex min-h-[108px] flex-col justify-between rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
        ...
        <article className="col-span-2 flex min-h-[108px] flex-col justify-between rounded-xl border border-neutral-200 bg-white p-4 shadow-sm lg:col-span-1">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-neutral-500">Top station</p>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-6">
          <div className="min-w-0 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-5">
            <h2 className="text-sm font-semibold text-ink">Station performance</h2>
            <p className="mt-0.5 text-xs text-neutral-500">Composite score by forecourt / depot</p>
        ...
        <div className="min-w-0 w-full">
          <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm">
            <div className="border-b border-neutral-200 bg-neutral-50 p-4 sm:p-5">
              <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-neutral-500">Filters</p>
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-neutral-300 bg-white py-2.5 pl-9 pr-3 text-sm focus:border-transparent focus:ring-2 focus:ring-primary-500/30"
                  />
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                  <select
                    ...
                    className="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary-500/30 lg:col-span-2"
                  >
            ...
            <div className="border-b border-neutral-200 px-4 py-3 sm:px-5">
              <h2 className="text-sm font-semibold text-ink">Employee KPIs</h2>
              <p className="mt-0.5 text-xs text-neutral-500">
                {filteredRows.length} employee{filteredRows.length === 1 ? '' : 's'} · {period}
              </p>
            </div>

            {filteredRows.length === 0 ? (
              <div className="table-empty-state min-h-[240px] border-0 px-4">
                <TrendingUp className="h-10 w-10 text-neutral-300" aria-hidden />
                <p className="text-sm font-medium text-neutral-700">No employees match these filters</p>
                <p className="text-sm text-neutral-500">Try clearing search or widening station / department.</p>
              </div>
            ) : (
                  ...
                  <tbody>
                    {filteredRows.map((row) => (
                      <tr key={row.id}>
                        ...
                        <td>{row.station}</td>
                        <td>{row.role}</td>
                        <td data-numeric="true">{row.status === 'Not Reviewed' ? '—' : row.throughput}</td>
                        <td data-numeric="true">{row.status === 'Not Reviewed' ? '—' : `${row.attendancePct}%`}</td>
                        <td data-numeric="true">{row.status === 'Not Reviewed' ? '—' : row.customerScore.toFixed(1)}</td>
                        <td data-numeric="true">{row.status === 'Not Reviewed' ? '—' : row.overall}</td>
                        <td>
                          <span className={`badge-status ${statusBadgeClass(row.status)}`}>{row.status}</span>
                        </td>
                        <td className="text-right">
                          <div className="flex flex-wrap items-center justify-end gap-2">
                            <button type="button" className="btn-secondary px-3 py-1.5 text-xs" onClick={() => setViewEmployee(row)}>
                              View
                            </button>
                            {(row.status === 'Not Reviewed' || row.status === 'Needs Improvement') && (
                              <button
                                type="button"
                                className="rounded-md bg-amber-400 px-3 py-1.5 text-xs font-medium text-neutral-900 hover:bg-amber-500"
                                onClick={() => openStartReview(row)}
                              >
                                Start Review
                              </button>
                            )}
