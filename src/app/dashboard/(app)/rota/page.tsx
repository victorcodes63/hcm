...
export default function RotaPage() {
  ...
  function exportFailedRowsCsv() {
    ...
    for (const f of bulkFailures) {
      ...
      const startOrTemplate = isTemplate ? f.payload.shiftTemplateId : f.payload.startsAt;
      const end = isTemplate ? '' : f.payload.endsAt;
      const br = isTemplate ? '' : String(f.payload.breakMinutes);
      const row = [
        f.source,
        f.employeeId,
        empNo,
  ...
  return (
    <div className="w-full min-w-0">
      <div className="mb-6 sm:mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-primary-900 mb-1">Rota planner</h1>
          <p className="text-neutral-600 text-sm sm:text-base">
            Build shift templates, plan periods, assign staff, scan conflicts, and import from CSV.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="h-10 rounded-lg border border-neutral-300 px-3 text-sm bg-white inline-flex items-center gap-2"
          >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-5 sm:col-span-2">
          <div className="flex items-center justify-between gap-2 text-primary-800 font-semibold mb-2">
            <div className="flex items-center gap-2">
                  ...
                  <div className="overflow-auto rounded border border-neutral-200 bg-white">
                    <table className="w-full text-xs min-w-[900px]">
                      <thead className="bg-neutral-50 text-neutral-600">
                        <tr>
                          <th className="text-left px-2 py-2 w-44">Employee</th>
                          {weekDays.map((d) => {
