  ...
  return (
    <div className="w-full min-w-0 space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-primary-900">Public holidays</h1>
          <p className="text-sm text-neutral-600">System-wide holiday calendar for automatic overtime rates.</p>
        </div>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-neutral-100">
                    <td className="px-2 py-2">{r.recurring ? `${r.recurDay} ${MONTH_NAMES[(r.recurMonth ?? 1) - 1]}` : (r.date?.slice(0, 10) ?? '—')}</td>
                    <td className="px-2 py-2">{r.name}</td>
                    <td className="px-2 py-2">{r.recurring ? 'Recurring' : 'Specific'}</td>
                    <td className="px-2 py-2">{r.isActive ? 'Active' : 'Inactive'}</td>
