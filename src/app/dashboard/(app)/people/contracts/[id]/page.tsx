...
export default function PeopleContractDetailPage() {
  ...
  if (loading) {
    return (
      <div className="w-full min-w-0 rounded-xl border border-neutral-200 bg-white p-6 text-sm text-neutral-500">
        Loading contract...
      </div>
    );
  }
    ...
    <div className="w-full min-w-0">
      ...
      <div className="rounded-xl border border-neutral-200 bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-primary-900">{contract.title || 'Contract'}</h1>
            <p className="mt-1 text-sm text-neutral-600">
              {contract.contractType === 'consultant' ? 'Consultant doctor contract' : 'Employee contract'} · Ref:{' '}
              {contract.reference || '—'}
            </p>
          </div>
          {status ? (
